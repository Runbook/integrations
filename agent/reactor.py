#!/usr/bin/env python
#####################################################################
#### reactor.py | CloudRoutes Local Reactor
#### -------------------------
#### This wrapper can be used to execute reactions locally when monitors fail. The agent
#### will query the CloudRoutes API for the defined monitors. If those monitors are in
#### a "failed" status the wrapper will execute the scripts defined in the YAML config
#### file.
#### -------------------------
#### Sample Config:
####
#### You can find a sample configuration in our GitHub Repository
#### https://github.com/CloudRoutes/integrations/blob/master/wrappers/config/reactor.sample
#### -------------------------
...
def crAPI(url, check_key):
  ...
  if debug:
    print "-" * 25
    print("[Reactor Wrapper Debug] Sent Request to CloudRoutes and got return code: %r") % req.status_code
    print "-" * 25
    print(req.text)
    print "-" * 25
  ...
  if req.status_code == 200:
    ...
    if "success" in req.text:
      ...
      print("[Reactor Wrapper] Error checking status from CloudRoutes for url: %s") % url
      return False
  else:
    print("[Reactor Wrapper] Error checking status from CloudRoutes for url: %s") % url
    return False
