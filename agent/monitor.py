#!/usr/bin/env python
#####################################################################
#### monitor.py | CloudRoutes Monitor Wrapper
#### -------------------------
#### This script can be used to integrate your own monitoring systems with CloudRoutes
...
#### this wrapper will read the exit code and send an appropriate API request to
#### CloudRoutes API.
#### -------------------------
#### Exit Codes:
#### 
#### When the sub shell returns with a 0 exit code the monitor will send a "Healty" API
#### call to CloudRoutes. If the sub shell returns with any other exit code the monitor
#### will send a "Failed" API call.
#### -------------------------
#### Sample Config:
####
#### You can find a sample configuration in our GitHub Repository
#### https://github.com/CloudRoutes/integrations/blob/master/wrappers/config/monitor.sample
#### -------------------------
#### CloudRoutes - Automated Incident Response | https://cloudrout.es
#####################################################################
...
def crAPI(url, check_key, action):
  ''' Call CloudRoutes API to change or update the monitors status '''
  ...
  try:
    ...
    print("[Reactor Wrapper] Error communicating with CloudRoutes, got SSL certificate error")
    print("Exiting...")
    sys.exit(2)
  ...
  if debug:
    print "-" * 25
    print("[Monitor Wrapper Debug] Sent Request to CloudRoutes and got return code: %r") % req.status_code
    print "-" * 25
    print(req.text)
    print "-" * 25
  ...
  if req.status_code == 200:
    ...
    if "success" in req.text:
      print("[Monitor Wrapper] Sent %s action to CloudRoutes") % action
      return True
    else:
      print("[Monitor Wrapper] Error sending %s action to CloudRoutes") % action
      return False
  else:
    print("[Monitor Wrapper] Could not send %s action to CloudRoutes") % action
    print req.text
    return False
